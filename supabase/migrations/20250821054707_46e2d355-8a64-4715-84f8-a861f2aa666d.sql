-- Función para enviar notificación cuando se seleccionan canciones para un servicio
CREATE OR REPLACE FUNCTION notify_song_selection()
RETURNS TRIGGER AS $$
DECLARE
    service_info RECORD;
    all_songs TEXT;
...
BEGIN
    ...
    SELECT string_agg(s.title || ' - ' || COALESCE(s.artist, 'Artista desconocido'), E'\n') INTO all_songs
...
-- Crear trigger para notificar cuando se agreguen canciones a servicios
DROP TRIGGER IF EXISTS trigger_notify_song_selection ON service_songs;
CREATE TRIGGER trigger_notify_song_selection
    AFTER INSERT ON service_songs
    FOR EACH ROW
    EXECUTE FUNCTION notify_song_selection();