  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        // ✅ Auto-publicar inmediatamente sin sincronización previa
      ...
      {allTracks.map((track) => {
        ...
        return (
          <div key={track.id} className="bg-card border rounded-lg p-4">
            ...
            <div id={`track-container-${track.id}`} className="mb-3 relative h-20 bg-muted/20 rounded overflow-visible">
              <div
              ...
              >
                <div id={`waveform-${track.id}`} className="w-full h-full" />
              </div>
            </div>
