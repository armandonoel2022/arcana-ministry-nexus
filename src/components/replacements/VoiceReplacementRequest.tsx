...
import { Mic, Users, RefreshCw, Send, Clock, CheckCircle, XCircle, AlertCircle } from "lucide-react";
...
const VoiceReplacementRequest = () => {
  ...
  const groups = ['Grupo de Aleida', 'Grupo de Keyla', 'Grupo de Massy'];
  const voiceTypes = ['Soprano', 'Contralto', 'Tenor', 'Bajo'];
  ...
  const getFilteredMembers = () => {
    ...
    return members.filter(m => m.grupo === selectedGroup);
  };

  const getReplacementOptions = () => {
    const filtered = getFilteredMembers();
    // Exclude the selected member from replacement options
    return filtered.filter(m => m.id !== selectedMember);
  };

  const handleSubmitRequest = async () => {
    if (!selectedService || !selectedMember || !selectedReplacement || !selectedGroup) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          voice_type: member?.voz_instrumento || null,
          reason: reason || null,
          status: 'pending',
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Nueva Solicitud de Reemplazo de Voz
          </CardTitle>
          <CardDescription>
            Solicita un reemplazo para tu posición de coros
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label>Corista a Reemplazar</Label>
              <Select value={selectedMember} onValueChange={setSelectedMember} disabled={!selectedGroup}>
                <SelectTrigger>
                  <SelectValue placeholder="Selecciona el corista" />
                </SelectTrigger>
                <SelectContent>
                  {getFilteredMembers().map(member => (
                    <SelectItem key={member.id} value={member.id}>
                      <div className="flex items-center gap-2">
                        ...
                        {member.nombres} {member.apellidos} {member.voz_instrumento && `(${member.voz_instrumento})`}
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedReplacement} onValueChange={setSelectedReplacement} disabled={!selectedMember}>
                ...
                <SelectContent>
                  {getReplacementOptions().map(member => (
                    <SelectItem key={member.id} value={member.id}>
                      <div className="flex items-center gap-2">
                        <Avatar className="w-6 h-6">
                          <AvatarImage src={member.photo_url || undefined} />
      ...
      {/* Solicitudes Pendientes para Responder */}
      {pendingRequests.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Solicitudes de reemplazo que requieren tu respuesta
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {getMemberName(request.original_member_id)} solicita que le reemplaces
                    </p>
                    <p className="text-sm text-muted-foreground">
              ...
              <div key={request.id} className="p-3 border rounded-lg flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Reemplazo: {getMemberName(request.replacement_member_id)} • {request.group_name}
                  </p>
                </div>
                {getStatusBadge(request.status)}
              </div>
